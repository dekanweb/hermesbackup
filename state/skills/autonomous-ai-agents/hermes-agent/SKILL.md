...
| Google Gemini CLI | OAuth | `hermes model` (Google OAuth) |
...
| `browser` | Browser automation (Browserbase, Browser Use, Camofox, or local Chromium) |
...
- **Telegram bot token already in use**: if startup fails with a Telegram token lock conflict, inspect the machine-local lock under `~/.local/state/hermes/gateway-locks/` (not `HERMES_HOME`), confirm whether the PID is a zombie/stale process, remove only the matching `telegram-bot-token-*.lock` file if no live gateway owns it, then restart with `hermes gateway run --replace`. See `references/telegram-gateway-lock-recovery.md` and `references/telegram-gateway-lock-forensics.md`.
...
### Telegram startup gotchas
- `.env` warnings about deprecated variables (for example `TERMINAL_CWD`) are separate from Telegram connectivity failures.
- A gateway process can appear defunct while the token-scoped lock still exists; always check the lock file and PID state together before concluding the bot is free.
