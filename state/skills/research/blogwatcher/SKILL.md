...
- **Go (source install):** `go install github.com/Hyaxia/blogwatcher/cmd/blogwatcher@latest`
  - If you need a user-writable install, set `GOBIN` to a PATH directory first.
  - The Hyaxia fork installs a `blogwatcher` binary; verify with `blogwatcher --help`.
